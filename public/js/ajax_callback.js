...
$(document).ready(function() {
	...
	if (jQuery(".popular-new-blog-wrapper").length > 0) {
		$('.popular-new-blog-wrapper').imagesLoaded(function() {
			// init Isotope
			var $container = $('.popular-new-blog-wrapper').isotope({
				itemSelector: '.popular-news-item',
				layoutMode: 'fitRows',
			});

			// bind filter button click
			$('#filters_blog').on('click', 'button', function() {
				var filterValue = $(this).attr('data-filter');
				// use filterFn if matches value
				$container.isotope({
					filter: filterValue
			...
			function loadMore(toShow) {
				...
				//when no more to load, hide show more button
				if (hiddenElems.length == 0) {
					jQuery("#see-load-more-blog").hide();
				} else {
					jQuery("#see-load-more-blog").show();
				};

			}
			//when load more button clicked
			$("#see-load-more-blog").on('click', function() {
				if ($('#filters_blog').data('clicked')) {
					//when filter button clicked, set initial value for counter
					counter = initShow;
					$('#filters_blog').data('clicked', false);
				} else {
					counter = counter;
				};
			...
			//when filter button clicked
			$("#filters_blog").on('click', function() {
				$(this).data('clicked', true);
